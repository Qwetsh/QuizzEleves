// Seede les questions « World of Warcraft » (CSV « ; » — champs guillemetés
...
// subject='world_of_warcraft'. t = sous_theme (lore, quêtes, PNJ, localisations,
// donjons & raids). Les colonnes note_verif/type_question/image_ref (au-delà de
// la 10e) sont ignorées par le parser.
//   node scripts/seed-world-of-warcraft.mjs
...
const file = path.join(process.cwd(), 'scripts', 'generated', 'pool', 'world_of_warcraft_source.csv');
...
lines.forEach((line, i) => {
  ...
  rows.push({
    pool: 'cycle4', subject: 'world_of_warcraft', level: null, q: q.trim(),
...
{ const { error } = await sb.from('quete_questions').delete().eq('subject', 'world_of_warcraft'); if (error) { console.error('del:', error.message); process.exit(1); } }
...
console.log(`✓ ${rows.length} questions « World of Warcraft » insérées.`);
