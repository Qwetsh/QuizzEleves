// Seede les questions « Film d'horreur » (CSV « ; » fourni avec distracteurs +
// explication + difficulté 1-5 + généralité 1-5 déjà remplis). Mélange les
// positions de réponse. Idempotent : delete-then-insert subject='film_horreur'.
// t = sous_theme (slashers, J-horror, folklore…). Robuste aux en-têtes résiduels.
//   node scripts/seed-film-horreur.mjs
...
const file = path.join(process.cwd(), 'scripts', 'generated', 'pool', 'film_horreur_source.csv');
const lines = readFileSync(file, 'utf8').split(/\r?\n/).filter((l) => l.trim());
lines.shift(); // en-tête

// Champs entre guillemets, séparés par « ; » ; aucun champ ne contient de guillemet.
const parseLine = (line) => { const out = []; const re = /"([^"]*)"/g; let m; while ((m = re.exec(line))) out.push(m[1]); return out; };
...
lines.forEach((line, i) => {
  ...
  rows.push({
    pool: 'cycle4', subject: 'film_horreur', level: null, q: q.trim(),
...
{ const { error } = await sb.from('quete_questions').delete().eq('subject', 'film_horreur'); if (error) { console.error('del:', error.message); process.exit(1); } }
...
console.log(`✓ ${rows.length} questions Film d'horreur insérées.`);
