// Seede les questions Pokémon (CSV « ; » fourni avec distracteurs + explication +
// difficulté 1-5 + généralité 1-5 déjà remplis). Mélange les positions de réponse.
// Idempotent : delete-then-insert subject='pokemon'. t = sous_theme (Gen 1…, mécaniques).
...
const file = path.join(process.cwd(), 'scripts', 'generated', 'pool', 'pokemon_source.csv');
...
lines.forEach((line, i) => {
  ...
  rows.push({
    pool: 'cycle4', subject: 'pokemon', level: null, q: q.trim(),
...
{ const { error } = await sb.from('quete_questions').delete().eq('subject', 'pokemon'); if (error) { console.error('del:', error.message); process.exit(1); } }
...
console.log(`✓ ${rows.length} questions Pokémon insérées.`);
