// Seede les questions « Super-héros » (CSV « ; » — champs guillemetés SEULEMENT
// s'ils contiennent un « ; »). Parser CSV à états (respecte les guillemets),
// mélange les positions de réponse. Idempotent : delete-then-insert
// subject='super_heros'. t = sous_theme (MCU, Batman, X-Men…).
//   node scripts/seed-super-heros.mjs
...
const file = path.join(process.cwd(), 'scripts', 'generated', 'pool', 'super_heros_source.csv');
...
lines.forEach((line, i) => {
  ...
  rows.push({
    pool: 'cycle4', subject: 'super_heros', level: null, q: q.trim(),
...
{ const { error } = await sb.from('quete_questions').delete().eq('subject', 'super_heros'); if (error) { console.error('del:', error.message); process.exit(1); } }
...
console.log(`✓ ${rows.length} questions Super-héros insérées.`);
