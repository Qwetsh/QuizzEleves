...
2. **Audio** ⏳ **À FAIRE** (pour hymnes + cris) : colonne `audio` + upload dans le
   seed + bouton lecteur dans `QuestionModal` et `ControllerView` + inclusion sûre
   dans le payload de session.

## 7. État & ordre de bataille

- ✅ **Drapeaux** : 388 questions image fusionnées dans le thème `drapeaux_symboles`.
- ✅ **« Qui est ce Pokémon ? »** (prototype de bout en bout) : `render`/`hard`
  (migration), rendu silhouette + jingle, cassette `pokemon_silhouette` (hard),
  `scripts/seed-pokemon-silhouette.mjs` (PokéAPI Gen 1, artwork ré-hébergé, noms FR,
  distracteurs même type). **151 questions en base.**
- ⏳ Ensuite, par impact décroissant : **peinture** (The Met), **cinéma** (TMDB),
  **monuments** (Wikidata), **animaux** photo+cris (GBIF + xeno-canto → brique audio),
  **gastronomie** (TheMealDB), **hymnes** (Commons, brique audio).
